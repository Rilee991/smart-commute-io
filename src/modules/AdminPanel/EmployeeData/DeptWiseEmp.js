...
const DeptWiseEmp = ({ deptEmpChartData, deptEmpTableData }) => {
    ...
    return (
        <Card>
            ...
            <CardContent>
                {showEmployeeChart ? (
                    <BarChart xAxisLabel="Department" yAxisLabel="Employees" data={deptEmpChartData} bars={Object.keys(deptEmpChartData[0] || {}).filter(item => item !== 'department')} xAxisKey="department" />
                ) : (
                    <Table>
                        <TableHeader>
                            <TableRow>
